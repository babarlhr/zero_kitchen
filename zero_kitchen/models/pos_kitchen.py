...
class PosOrder(models.Model):
    ...
    kitchen_ref = fields.Many2one('pos.kitchen', string='Kitchen Ref')
    ...
    def create_from_ui(self, orders):
        ...
        for tmp_order in orders_to_save:
            ...
            if pos_order.kitchen_ref:
                pos_order.kitchen_ref.write({'state': 'confirmed'})
                kitchen_ids.append(pos_order.kitchen_ref.id)
        ...
        return order_ids, kitchen_ids

    @api.model
    def _order_fields(self, ui_order):
        order_fields = super(PosOrder, self)._order_fields(ui_order)
        kitchen_id = False
        if 'kitchen_ref' in ui_order:
            if ui_order['kitchen_ref']:
                kitchen_id = ui_order.get('kitchen_ref')['id']
        order_fields['kitchen_ref'] = kitchen_id
        return order_fields
...
class PosKitchen(models.Model):
    ...
    @api.model
    def _order_fields(self, ui_order):
        process_line = partial(self.env['pos.kitchen.line']._order_line_fields)
        return {
    ...
    date_kitchen = fields.Datetime(string='Kitchen Date', readonly=True, index=True, default=fields.Datetime.now)
    ...
    lines = fields.One2many('pos.kitchen.line', 'order_id', string='Order Lines', copy=True)
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('pos.kitchen') or '/'
        return super(PosKitchen, self).create(vals)


class PosKitchenLine(models.Model):
    _name = "pos.kitchen.line"
    ...
    order_id = fields.Many2one('pos.kitchen', string='Order Ref', ondelete='cascade')
...
class PosConfig(models.Model):
    ...
    enable_kitchen = fields.Boolean('Enable Kitchen Screen')
